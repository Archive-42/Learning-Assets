...
(function (factory) {
    ...
    tablesorter: new (function () {
      ...
      ts.getElementText = function (c, node, cellIndex) {
        ...
        if (typeof t === "string") {
          ...
          return $.trim(
            (t === "basic"
              ? $node.attr(c.textAttribute) || node.textContent
              : node.textContent) ||
              $node.text() ||
              ""
          );
          ...
          if (typeof t === "function") {
            ...
            typeof (te = ts.getColumnData(c.table, t, cellIndex)) === "function"
          ) {
      ...
      function detectParserForColumn(table, rows, rowIndex, cellIndex) {
        ...
        while (--i >= 0) {
          ...
          if (
            ...
            cur.id !== "text" &&
            cur.is &&
            cur.is(nodeValue, table, node, $node)
          ) {
            return cur;
          }
        }
        ...
        c.$headers = $(
          $.map($(table).find(c.selectorHeaders), function (elem, index) {
            ...
            if (typeof lock !== "undefined" && lock !== false) {
              elem.order = elem.lockedOrder = formatSortingOrder(lock)
                ? [1, 1, 1]
                : [0, 0, 0];
            }
              ...
              if ($tb.length && tbdy >= 0) {
                ...
                if (typeof c.extractors[icell].id === "undefined") {
                  ...
                  t = c.extractors[icell].format(
                    ts.getElementText(c, cell, icell),
                    table,
                    cell,
                    icell
                  );
                ...
                for (j = 0; j < l; j++) {
                  if (typeof c.extractors[j].id === "undefined") {
                    ...
                    t = c.extractors[j].format(
                      ts.getElementText(c, $row[i].cells[j], j),
                      table,
                      $row[i].cells[j],
                      j
                    );
          ...
          .bind(
            ...
            function (e, external) {
              ...
              cell = $.fn.closest
                ...
                : $target.parents("td,th").filter(":first");
              // prevent sort being triggered on form elements
              if (
                /(input|select|button|textarea)/i.test(e.target.tagName) ||
      ...
      ts.applyWidget = function (table, init, callback) {
        ...
        wd = new RegExp(
          "\\s" + c.widgetClass.replace(/\{name\}/i, "([\\w-]+)") + "\\s",
          "g"
        );
            ...
            if (widgets[indx]) {
              if (init || !c.widgetInit[widgets[indx].id]) {
                ...
                if ("init" in widgets[indx]) {
                  ...
                  if (c.debug) {
                    ts.benchmark(
                      "Initializing " + widgets[indx].id + " widget",
                      time2
                    );
                  }
                }
      ...
      ts.refreshWidgets = function (table, doAll, dontapply) {
        ...
        for (indx = 0; indx < len; indx++) {
          if (
            widgets[indx] &&
            widgets[indx].id &&
            (doAll || $.inArray(widgets[indx].id, cw) < 0)
          ) {
  ...
  ts.addParser({
    ...
    format: function (s, table, cell) {
      return (
        $(cell)
          .find("img")
          .attr(table.config.imgAttr || "alt") || s
      );
    },
    parsed: true, // filter widget flag
    type: "text",
  ...
  ts.addParser({
    ...
    format: function (s, table) {
      var date = s
        ? new Date("2000/01/01 " + s.replace(/(\S)([AP]M)$/i, "$1 $2"))
        : s;
      return date instanceof Date && isFinite(date) ? date.getTime() : s;
    },
    type: "numeric",
