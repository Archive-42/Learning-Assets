...
test("Binary and Sequential search on array of numbers", (assert) => {
  ...
  assert.ok(
    result,
    result >= 0
      ? `${number} is present in the set and is in the position ${result}`
      : "not generated number"
  );
  assert.ok(
    search.minValue,
    `The minimum value is: ${search.minValue(dataStore)}`
  );
  assert.ok(
    search.maxValue,
    `The maximum value is: ${search.maxValue(dataStore)}`
  );
  ...
  assert.ok(
    result,
    result >= 0
      ? `${number} is present in the set and is in the position ${result} and has ${count} occurrences`
      : "not generated number"
  );
});
...
test("Binary and Sequential search on text file", (assert) => {
  let data = fs
    .readFileSync("./13-chapter-Searching-Algorithms/paragraph.txt", "utf8")
    .trim()
    .split(" ");
  ...
  assert.ok(
    position,
    `${word} is present in the set and is in the position ${position}`
  );
  assert.ok(
    position,
    `Sequential search took ${nsTime(end) / 1e9} milliseconds.`
  );
