...
const gradeObj = [
  90, 89, 75, 90, 89, 75, 90, 89, 75, 90, 89, 75, 90, 89, 75, 90, 89, 75,
];
...
console.log(
  `displaying forward: ${arrayWords.reduce((total, word) => total + word)}`
);
console.log(
  `displaying backward: ${arrayWords.reduceRight(
...
class weekTemps {
  ...
  displayMonthAvg() {
    ...
    this.dataStore.forEach((week) => week.forEach((day) => (sum += day)));
  ...
  displayAllWeekAvg() {
    ...
    this.dataStore.forEach((week, count) => {
      ...
      console.log(
        `Week ${count + 1} Temp Avg: ${(sum / week.length).toFixed(2)}`
      );
      sum = 0;
    });
  }
...
const weekTempsProto = {
  ...
  displayMonthAvg() {
    let sum = this.dataStore.reduce(
      (weeks, week) => weeks + week.reduce((days, day) => days + day, 0),
      0
    );
    return (sum / (this.dataStore.length * 4)).toFixed(2);
  },
