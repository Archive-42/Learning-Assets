...
app.post("/new", async (req, res) => {
  try {
    ...
    const new_entry = await pool.query(
      `INSERT INTO Earthquake(Name, Magnitude, Occurrence, LONG, LAT)
        VALUES($1, $2, $3, $4, $5)`,
      [name, magnitude, occurrence, long, lat]
    );
...
app.delete("/delete/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const response = await pool.query("DELETE FROM Earthquake WHERE ID = $1", [
      id,
    ]);
