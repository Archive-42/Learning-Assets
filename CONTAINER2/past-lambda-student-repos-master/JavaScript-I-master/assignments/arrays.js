...
let lastCar = 0;
console.log(
  `Our last car is a ${inventory[lastCar].car_year} ${inventory[lastCar].car_make} ${inventory[lastCar].car_model}.`
);
//OR
let lastCar = 0;
function finalCar(number) {
  return `Our last car is a ${inventory[number].car_year} ${inventory[number].car_make} ${inventory[number].car_model}`;
}
finalCar(lastCar);
...
//OR to re-write a for loop that gets it without carYears
let oldCars = [];
for (i = 0; i < inventory.length; i++) {
...
for (i = 0; i < inventory.length; i++) {
  if (inventory[i].car_make === "BMW" || inventory[i].car_make === "Audi") {
    BMWAndAudi.push(
      `We have a ${inventory[i].car_year} ${inventory[i].car_make} ${inventory[i].car_model}`
    );
  }
}
