    ...
    (props) => {
      if (status != null) {
        ...
        if (status.usertype === "helper") {
          ...
          setTicketURL(
            `https://devdesk2eli.herokuapp.com/api/tickets/helpers/${status.id}`
          );
          ...
          setTicketURL(
            `https://devdesk2eli.herokuapp.com/api/tickets/${status.id}`
          );
        }
...
if (UserForm.loggedIn === true) {
  ...
  FormikForm = withFormik({
    ...
    handleSubmit(
      values,
      { status, setStatus, resetForm, setErrors, setSubmitting }
    ) {
