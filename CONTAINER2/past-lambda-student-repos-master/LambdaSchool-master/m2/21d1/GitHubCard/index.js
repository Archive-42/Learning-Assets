...
async function passData(desiredUN) {
  ...
  calendarOptions.summary_text = await GitHubCalendar(
    ".calendar",
    `${desiredUN}`
  );
  calendarOptions.global_stats = await GitHubCalendar(
    ".calendar",
    `${desiredUN}`
  );
  calendarOptions.responsive = await GitHubCalendar(
    ".calendar",
    `${desiredUN}`
  );
...
async function arrayIterate() {
  ...
  */
  ...
  await friendsArray.forEach((item) => {
    passData(item);
  });
  await friendsArray.forEach((item) => {
    addContributions(item);
  });
}
