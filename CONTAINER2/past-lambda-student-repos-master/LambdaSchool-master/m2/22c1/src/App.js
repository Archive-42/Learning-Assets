...
function App() {
  ...
  useEffect(() => {
    ...
    var maxMonth = 12;
    var randomMonth =
      minMonth + Math.round(Math.random() * (maxMonth - minMonth));
    var minDay = 1;
    var maxDay = 28;
    ...
    function refresh() {
      ...
      axios
        ...
        .then((response) => {
          ...
          setExplanation1(
            `${response.data.photos[0].rover.name}, ${response.data.photos[0].rover.status}`
          );
        })
        .catch((error) => {
          console.log(error);
  ...
  return (
    <div className="App">
      ...
      <div id="nasa1" className="nasa">
        <NasaH2>Random Images from sols 700 to 1000:</NasaH2>
      </div>
      <div id="nasa2" className="nasa">
        <NasaH3>
          <NasaA href={imgURL1} target="_blank">
            IMG URL
          </NasaA>
        </NasaH3>
      </div>
      <div id="nasa4" className="nasa">
        <NasaH3>ID Number: </NasaH3>
        {copyright1}
      </div>
      <div id="nasa5" className="nasa">
        <NasaH3>Status: </NasaH3>
        {explanation1}
      </div>
