...
server.get("/", (req, res) => {
  db("accounts")
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .json({ errorMessage: "The account list could not be retrieved." });
    });
});
//get one account
  ...
  db("accounts")
    ...
    .then((account) => {
      if (account) {
        ...
        res
          .status(404)
          .json({ errorMessage: "No account found with that I.D." });
      }
    })
    .catch((err) => {
      console.log(err);
...
server.delete("/:id", (req, res) => {
  ...
  db("accounts")
    ...
    .catch((error) => {
      res
        .status(500)
        .json({ errorMessage: `account ${accountID} not deleted.`, error });
    });
});
// create one account
