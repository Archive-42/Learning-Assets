...
let fullName = [];
let wholeName = runners.forEach(
  (fullNameCallback = (item) => {
...
let donationAverage = runners.reduce(
  (averageAmount = (total, amount, index, array) => {
    return ticketPriceTotal / array.length;
  })
);
...
let bookletCopy = runners.map(
  (runnerStatement = (item) => {
...
console.log(
  `Order ${smallShirts.length} small shirts, ${mediumShirts.length} medium shirts, ${largeShirts.length} large shirts, ${extraLShirts.length} XL shirts, and ${twoXLShirts.length} 2XL shirts.`
);
