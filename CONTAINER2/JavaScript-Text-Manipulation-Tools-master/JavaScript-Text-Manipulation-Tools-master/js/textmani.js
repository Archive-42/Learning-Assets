...
function findAndReplace() {
  ...
  if ($("#replaceAll").is(":checked")) {
    //Checking if the option for Replace is checked
...
function compareLists() {
  ...
  for (let i = 0; i < originalList.length; i++) {
    ...
    if (index > -1) {
      //If it exists then the answer will be more than -1 and if it dosen't then answer will be -1
  ...
  $("#InputandOutput").html(
    '<label><h5>Output Text:</h5></label><textarea class = "form-control" id ="inputText" rows ="10">' +
      listToBeDupesDeletedFrom +
      "</textarea>"
  );
}
