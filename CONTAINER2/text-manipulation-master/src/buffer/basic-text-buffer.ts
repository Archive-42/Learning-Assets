...
export class BasicTextBuffer implements TextBuffer {
  ...
  replaceRange(range: TextRange, text: string): TextRange | undefined {
    ...
    if (
      this.table[range.start.line] !== undefined &&
      this.table[endLine] !== undefined
    ) {
        ...
        if (n === 1) {
          ...
          if (i === 0) {
            this.insertText(
              lastChangedCol,
              lastChangedLine,
              table[i].join("") + this.lineDelimiter
            );
          } else {
  ...
  removeRange(range: TextRange): TextPosition {
    ...
    return {
      line: line,
      column: prefixChars.length > 0 ? prefixChars.length : startCol,
    };
  }
