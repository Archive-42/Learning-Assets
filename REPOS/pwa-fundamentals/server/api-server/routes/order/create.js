...
function beginNotificationSequence(api, order) {
  let p = api.notifications.pushWithDelay(2000, {
    title: order.name,
    body: "We've started shopping",
  });
  let orderItems = order.getorderItems();
  process.stdout.write("\n\nzzzITEMS" + JSON.stringify(orderItems) + "\n\n");
  orderItems.forEach((orderItem) => {
    p = p.then(() => {
      return api.notifications.pushWithDelay(1000, {
        title: order.name,
        body: `In cart: ${orderItem.qty}x ${orderItem.groceryItem.name}`,
      });
    });
  });
}
          ...
          .then((groceryItems) => {
            ...
            return Order.create({
              ...
              return Promise.all(
                ...
                let orderName = `Order placed at ${moment(
                  order.createdAt
                ).format("ddd, MMM D YY, h:mm a")}`;
                return order.update({
                  name: orderName,
