...
class AppHeader extends Component {
  ...
  onQrImageSelected(evt) {
    ...
    let file = files[0];
    return fileToImageBuffer(file).then((imageBuffer) => {
      let fileInput = document.getElementsByClassName("qr-input")[0];
      ...
      <header className="header">
        <Appbar className="mui--appbar-line-height">
          <div className="mui-container-fluid">
            ...
            <a
              className="appbar-icon cart-toggle mui--pull-right"
              onClick={doRightToggle}
            >
              {cartIcon}
              {numItemsInCart > 0 ? (
                <span className="cart-item-count-badge">{numItemsInCart}</span>
              ) : (
                ""
              )}
            </a>
            <a
              onClick={this.onQrButtonClicked}
              className="appbar-icon qr-reader mui--pull-right"
            >
              {qrIcon}
              <input
                className="qr-input"
                type="file"
                onChange={this.onQrImageSelected}
              />
            </a>
          </div>
        </Appbar>
