  ...
  render() {
    ...
    if (this.state.order) {
      let orderItems = this.state.order.orderItems.map((oi) => (
        <div
          key={oi.id}
          className="order-item mui-col-xs-6 mui-col-md-4 mui-col-lg-3"
        >
          <div className="mui-panel">
            <h3 className="item-name">{oi.groceryItem.name}</h3>
            <img
              src={"https://localhost:3100" + oi.groceryItem.imageUrl}
              className="item-img"
              alt={oi.groceryItem.name}
            />
      ...
      orderComponent = (
        <div>
          ...
          <div className="order-items md-row">{orderItems}</div>
        </div>
      );
    }
    return (
